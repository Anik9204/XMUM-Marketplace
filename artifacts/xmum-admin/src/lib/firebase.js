...
import { initializeFirestore, collection, query, orderBy, limit, getDocs, where, doc, updateDoc, deleteDoc, getDoc, onSnapshot, addDoc, serverTimestamp, } from "firebase/firestore";
...
export async function getPendingShopAds() {
    const q = query(collection(db, "shopAds"), where("status", "==", "pending"), orderBy("submittedAt", "desc"));
    const snap = await getDocs(q);
    return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
export async function getAllShopAds() {
    const q = query(collection(db, "shopAds"), orderBy("submittedAt", "desc"));
    const snap = await getDocs(q);
    return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
export function subscribePendingShopAds(callback, onError) {
    const q = query(collection(db, "shopAds"), where("status", "==", "pending"), orderBy("submittedAt", "desc"));
...
export function subscribeAllShopAds(callback, onError) {
    const q = query(collection(db, "shopAds"), orderBy("submittedAt", "desc"));
    return onSnapshot(q, (snap) => callback(snap.docs.map((d) => ({ id: d.id, ...d.data() }))), (err) => {
...
export async function notifyShopAdApproved(shopOwnerId, shopName, shopId) {
...
export async function notifyShopAdRejected(shopOwnerId, shopName, adminNote, shopId) {
    await sendNotification(shopOwnerId, {
...
export async function approveShopAd(adId, reviewedBy) {
...
export async function rejectShopAd(adId, reviewedBy, note) {
    await updateDoc(doc(db, "shopAds", adId), {
        status: "rejected",
        reviewedBy,
        reviewedAt: Date.now(),
...
export async function getShopInquiries(shopId) {
    const q = query(collection(db, "shopInquiries"), where("shopId", "==", shopId), orderBy("createdAt", "desc"));
    const snap = await getDocs(q);
    return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
...
export async function deleteShop(shopId) {
    ...
    const snaps = await Promise.all(relatedCollections.map((col) => getDocs(query(collection(db, col), where("shopId", "==", shopId))).catch(() => null)));
    // Delete Storage files — listing photos
    const storageJobs = [];
    const listingsSnap = snaps[0];
    if (listingsSnap) {
        for (const d of listingsSnap.docs) {
            for (const url of (d.data().photos ?? [])) {
                const path = storagePathFromUrl(url);
                if (path)
                    storageJobs.push(deleteObject(storageRef(storage, path)).catch(() => { }));
            }
        }
    }
    ...
    for (let i = 0; i < allDocs.length; i += BATCH_SIZE) {
        ...
        for (const d of allDocs.slice(i, i + BATCH_SIZE))
            batch.delete(d.ref);
        await batch.commit();
    }
...
export async function approveShop(shopId, shopOwnerId, shopName, _reviewedBy) {
...
export async function renewShopSubscription(shopId, shopOwnerId, shopName, _renewedBy) {
    const config = await getSubscriptionConfig();
    const now = Date.now();
...
export async function getPendingShops(limitCount = 100) {
    const q = query(collection(db, "shops"), where("approvalStatus", "==", "pending"), orderBy("createdAt", "asc"), limit(limitCount));
    const snap = await getDocs(q);
    return snap.docs.map((d) => {
        const data = d.data();
        return {
